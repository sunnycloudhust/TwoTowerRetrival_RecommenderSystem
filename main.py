...
def main():
    ...
    # Kết quả trả về gồm 2 DataFrame và 1 ma trận Genres
    ...
    print(f"user number: {prep.num_users}")
    print(f"movie number: {prep.num_movies}")
    ...
    train_loader = get_dataloader(train_df, genre_matrix, batch_size=HYPERPARAMS["batch_size"], shuffle=True)
    test_loader = get_dataloader(test_df, genre_matrix, batch_size=HYPERPARAMS["batch_size"], shuffle=False)
    ...
    # 5. Huấn luyện (Train)
    # ---------------------------------------------------------
    # Bạn có thể đổi resume=True nếu muốn train tiếp từ checkpoint cũ
